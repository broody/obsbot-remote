...
const App: React.FC = () => {
  ...
  const { status, segments, connected, toasts, removeToast, sendCommand, sendGimbalCommand } = useCamera(`ws://${host}:8080`);
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0a0a0c] text-zinc-100 font-sans overflow-hidden">
      {/* Header */}
      <header className="flex justify-between items-center px-6 py-3 bg-black/20 backdrop-blur-md border-b border-white/5 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold tracking-tight text-white leading-none">OBSBOT <span className="text-blue-500">Center</span></h1>
            <p className="text-[10px] text-zinc-500 font-mono mt-1">v2.0.0 • Server-Side Control</p>
          </div>
        </div>
          ...
          <div className="flex items-center gap-4 px-4 py-1.5 bg-zinc-900 rounded-full border border-white/5">
            ...
            <div className="flex items-center gap-2">
              <div className={cn(
                ...
                connected ? "bg-emerald-500 animate-pulse shadow-[0_0_8px_#10b981]" : "bg-red-500"
              )} />
      ...
      <main className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col p-6 gap-6">
            ...
            {connected ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-4">
                ...
                <div className="text-center">
                  ...
                  <p className="text-[10px] text-zinc-600 font-mono mt-1">Check if MediaMTX and obsbot-server are running</p>
                </div>
              </div>
            )}
          ...
          <div className="grid grid-cols-4 gap-4 shrink-0">
            {[
              { label: 'AI Mode', value: ['Off', 'Group', 'Human', 'Hand', 'Whiteboard', 'Desk'][status?.status?.aiMode || 0] || 'Off', icon: Zap, active: isAIEnabled },
              { label: 'Zoom', value: `${status?.zoom?.toFixed(1) || '1.0'}x`, icon: ZoomIn },
              { label: 'Tracking', value: isAIEnabled ? 'ACTIVE' : 'IDLE', icon: Move, active: isAIEnabled },
              { label: 'Audio', value: 'STREAMING', icon: Mic },
            ].map((item, i) => (
              <div key={i} className="bg-[#17171a]/70 backdrop-blur-md p-3 rounded-xl border border-white/5 flex items-center gap-3">
                <div className={cn("p-2 rounded-lg", item.active ? "bg-blue-500/20 text-blue-500" : "bg-white/5 text-zinc-400")}>
        ...
        <aside className="w-96 bg-[#17171a]/70 backdrop-blur-md border-l border-white/5 flex flex-col shrink-0">
          ...
          <nav className="flex bg-black/20 shrink-0 border-b border-white/5">
            <TabButton id="controls" label="Controls" icon={Move} />
            <TabButton id="ai" label="AI" icon={Zap} />
            <TabButton id="recordings" label="Library" icon={Save} />
          </nav>
              ...
              <>
                ...
                <section className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500">Gimbal Control</h3>
                    <div className="flex items-center gap-2">
                      <label className="flex items-center gap-1.5 cursor-pointer group">
                        <span className="text-[9px] font-bold text-zinc-500 group-hover:text-zinc-400 uppercase tracking-widest">Invert Y</span>
                        <input 
                          type="checkbox" 
                          checked={invertY}
                  ...
                  <div className="relative w-48 h-48 mx-auto">
                    <div className="absolute inset-0 rounded-full border-2 border-dashed border-white/5 flex items-center justify-center cursor-move">
                      <div className="w-40 h-40 rounded-full bg-white/[0.02] border border-white/10 shadow-inner flex items-center justify-center relative">
                        {/* Crosshairs */}
                        ...
                        <motion.div
                          drag
                          dragSnapToOrigin
                          dragConstraints={{ left: -55, right: 55, top: -55, bottom: 55 }}
                          dragElastic={0}
                          onDrag={handleJoystickDrag}
                          onDragEnd={handleJoystickReset}
                          style={{ x: joystickX, y: joystickY }}
                          className="w-12 h-12 bg-blue-600 rounded-full shadow-[0_0_20px_rgba(59,130,246,0.5)] cursor-grab active:cursor-grabbing flex items-center justify-center z-10"
                        >
                    ...
                    {/* Labels */}
                    <span className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-6 text-[10px] text-zinc-500 font-bold uppercase">Tilt</span>
                    <span className="absolute -right-8 top-1/2 -translate-y-1/2 text-[10px] text-zinc-500 font-bold uppercase rotate-90">Pan</span>
                  </div>
                </section>
                ...
                <section className="space-y-4 pt-6 border-t border-white/5">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-mono font-bold text-blue-400">{status?.zoom?.toFixed(1) || '1.0'}x</span>
                  </div>
                  <input 
                    type="range" min="1" max="4" step="0.1"
                    value={status?.zoom || 1}
                    onChange={(e) => handleZoom(parseFloat(e.target.value))}
                    className="w-full h-1 bg-zinc-800 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3.5 [&::-webkit-slider-thumb]:h-3.5 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-blue-500 [&::-webkit-slider-thumb]:shadow-[0_0_10px_rgba(59,130,246,0.5)] [&::-webkit-slider-thumb]:cursor-pointer"
                  />
                </section>
              ...
              <div className="space-y-6">
                ...
                <section className="space-y-4 pt-4 border-t border-white/5">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select 
                        ...
                        onChange={(e) => {
                          ...
                          sendCommand('ai-set-mode', { mode: newMode, subMode: status?.status?.aiSubMode || 0 }, { field: 'aiMode', value: newMode });
                        }}
                    ...
                    <div>
                      <label className="text-[10px] text-zinc-500 uppercase font-bold block mb-1">Sub-mode</label>
                      <select 
                        value={status?.status?.aiSubMode || 0}
                        onChange={(e) => {
                          const newSubMode = parseInt(e.target.value);
                          sendCommand('ai-set-mode', { mode: status?.status?.aiMode || 0, subMode: newSubMode }, { field: 'aiSubMode', value: newSubMode });
                        }}
                        className="w-full bg-white/5 border border-white/10 rounded-lg px-2 py-1.5 text-xs text-zinc-200 focus:outline-none focus:border-blue-500"
                      >
                  ...
                  <div>
                    ...
                    <select 
                      ...
                      onChange={(e) => {
                        ...
                        sendCommand('ai-set-tracking-speed', { speed: newSpeed }, { field: 'aiTrackerSpeed', value: newSpeed });
                      }}
                      className="w-full bg-white/5 border border-white/10 rounded-lg px-2 py-1.5 text-xs text-zinc-200 focus:outline-none focus:border-blue-500"
                    >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <label htmlFor="auto-zoom" className="text-xs text-zinc-300 cursor-pointer">Auto Zoom</label>
                  </div>
                </section>
                  ...
                  {[
                    ...
                    <label key={item.gesture} className="flex items-center gap-2 cursor-pointer">
                      <input 
                        ...
                        onChange={() => sendCommand('ai-set-gesture', { gesture: item.gesture, enabled: !item.isActive })}
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={cn(
                            ...
                            seg.type === 'video' ? "bg-black/30 text-blue-500" : "bg-black/30 text-amber-500"
                          )}>
                            {seg.type === 'video' ? <Video className="w-4 h-4" /> : <Mic className="w-4 h-4" />}
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              {seg.keep ? (
                                ...
                                <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Temp Buffer</span>
                              )}
